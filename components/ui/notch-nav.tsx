...
import { Clock3, Home, LayoutDashboard } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import { Show, SignInButton, SignUpButton, UserButton } from "@clerk/nextjs";

type IconName = "home" | "dashboard" | "history";
...
export function NotchNav({
  ...
  return (
    <div className="relative flex w-full items-center justify-center">
      ...
      {/* Auth controls — sit to the right, outside the centered pill */}
      {/* Auth controls — sit to the right, outside the centered pill */}
      <div className="absolute right-4 flex items-center gap-2 sm:right-6">
        <Show when="signed-out">
          <SignInButton mode="modal">
            <button className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-foreground/80 backdrop-blur-xl transition-colors hover:bg-white/10 hover:text-foreground">
              Sign in
            </button>
          </SignInButton>
          <SignUpButton mode="modal">
            <button className="rounded-full bg-white px-4 py-2 text-sm font-medium text-black shadow-[0_0_25px_rgba(255,255,255,0.15)] transition-all hover:bg-white/90 hover:shadow-[0_0_35px_rgba(255,255,255,0.25)]">
              Sign up
            </button>
          </SignUpButton>
        </Show>
        <Show when="signed-in">
          <UserButton
            appearance={{
              elements: {
                userButtonAvatarBox: "size-9 ring-1 ring-white/15",
              },
            }}
          />
        </Show>
